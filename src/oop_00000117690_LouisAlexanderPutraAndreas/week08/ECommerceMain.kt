package oop_00000117690_LouisAlexanderPutraAndreas.week08

fun main(){
    val rawApiData: List<Map<String, Any?>> = listOf(
        mapOf("id" to "E01", "name" to "Laptop", "type" to "ELECTRONIC", "warranty" to 24),
        mapOf("id" to "C01", "name" to "T-Shirt", "type" to "CLOTHING", "size" to "XL"),
        mapOf("id" to "E02", "name" to "Mouse", "type" to "ELECTRONIC", "warranty" to "NotAn Integer"), // Corrupted warranty
                mapOf("name" to "Ghost Item", "type" to "CLOTHING"), // Missing ID!
        mapOf("id" to "X01", "name" to "Unknown", "type" to "FOOD") // Unknown type
    )

    val parser = ApiParser()

    for (raw in rawApiData) {
        println("Memproses data: $raw")

        try {
            val parsedProduct = parser.parseProduct(raw)

            parsedProduct?.let { product ->
                parser.checkout(product)
            } ?: run {
                println("ℹTipe produk tidak dikenali atau diabaikan.")
            }

        } catch (e: IllegalArgumentException) {
            println("Peringatan Data Korup: ${e.message}")
        }

        println("--------------------------------------------------")
        // Output sesuai
    }

}